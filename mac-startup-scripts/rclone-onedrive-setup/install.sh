...
run_dry_run() {
	...
	if [ "$err" -eq 0 ]; then
		...
		echo "    (install_rclone_onedrive, mount_rclone_onedrive, unmount_rclone_onedrive, login_rclone_onedrive, logout_rclone_onedrive, check_rclone_onedrive, reset_rclone_onedrive, uninstall_rclone_onedrive, rclone_onedrive_help)"
		echo ""
		if [ -n "${ksvc:-}" ]; then
			if [ "$SKIP_KEYCHAIN" -eq 1 ]; then
...
install_shell_alias_hooks() {
	...
	for f in "${HOME}/.zshrc" "${HOME}/.bash_profile"; do
		...
		remove_rclone_setup_shell_hooks_from_file "$f"
		{
			...
			echo "install_rclone_onedrive() { command $(shell_single_quote "${dest}/install.sh") \"\$@\"; }"
			echo "mount_rclone_onedrive() { command $(shell_single_quote "${dest}/mount_onedrive.sh") \"\$@\"; }"
			echo "unmount_rclone_onedrive() { command $(shell_single_quote "${dest}/unmount_onedrive.sh") \"\$@\"; }"
			echo "login_rclone_onedrive() { command $(shell_single_quote "${dest}/login.sh") \"\$@\"; }"
			echo "logout_rclone_onedrive() { command $(shell_single_quote "${dest}/logout.sh") \"\$@\"; }"
			echo "check_rclone_onedrive() { command $(shell_single_quote "${dest}/check_rclone_config.sh") \"\$@\"; }"
			echo "reset_rclone_onedrive() { command $(shell_single_quote "${dest}/reset_rclone_config.sh") \"\$@\"; }"
			echo "uninstall_rclone_onedrive() { command $(shell_single_quote "${dest}/install.sh") --uninstall \"\$@\"; }"
			echo "rclone_onedrive_help() {"
			echo "	echo 'rclone-onedrive-setup shell helpers (functions, not aliases):'"
			echo "	echo ''"
			echo "	echo '  install_rclone_onedrive    → ${dest}/install.sh'"
			echo "	echo '  mount_rclone_onedrive      → ${dest}/mount_onedrive.sh'"
			echo "	echo '  unmount_rclone_onedrive   → ${dest}/unmount_onedrive.sh'"
			echo "	echo '  login_rclone_onedrive      → ${dest}/login.sh'"
			echo "	echo '  logout_rclone_onedrive    → ${dest}/logout.sh'"
			echo "	echo '  check_rclone_onedrive      → ${dest}/check_rclone_config.sh'"
			echo "	echo '  reset_rclone_onedrive      → ${dest}/reset_rclone_config.sh'"
			echo "	echo '  uninstall_rclone_onedrive → ${dest}/install.sh --uninstall'"
			echo "}"
			echo "$ALIAS_HOOK_END"
			echo ""
		} >>"$f"
...
print_install_summary() {
	...
	echo "    install_rclone_onedrive    → ${dest}/install.sh"
	echo "    mount_rclone_onedrive      → ${dest}/mount_onedrive.sh"
	echo "    unmount_rclone_onedrive   → ${dest}/unmount_onedrive.sh"
	echo "    login_rclone_onedrive      → ${dest}/login.sh"
	echo "    logout_rclone_onedrive    → ${dest}/logout.sh"
	echo "    check_rclone_onedrive      → ${dest}/check_rclone_config.sh"
	echo "    reset_rclone_onedrive      → ${dest}/reset_rclone_config.sh"
	echo "    uninstall_rclone_onedrive → ${dest}/install.sh --uninstall"
	echo "    rclone_onedrive_help        — print this list in any new shell"
	echo "    In this session run:  source ~/.zshrc   # or ~/.bash_profile, then: rclone_onedrive_help"
	echo ""
	...
	echo "    mount_rclone_onedrive $vol"
	echo "    login_rclone_onedrive      # uses EXTERNAL_VOLUME_NAME from config.sh"
